	...
	swal({ 
		...
		$.ajax({
			...
			success:function(result){
				...
				if (result.code == 1) {
					// 提示成功
					// 参数依次为 标题, 提示内容, 图标   swal("标题","内容","图标")
					swal("", "<h2>删除成功！</h2>", "success");
					// 执行成功的DOM操作 --- 根据ID删除指定的元素
					deleteDom(typeId);
				} else {
					// 提示用户失败
...
function openUpdateDialog(typeId) {
	...
	// 2、将当前要修改的tr记录的id和名称，设置到模态框的隐藏域和文本框中
	// 2.1 通过ID属性值，得到要修改的tr对象
	var tr = $("#tr_"+typeId);
	// 2.2 得到该tr的具体单元格的值
	var typeName = tr.children().eq(1).text();
	// 2.3 将类型名称赋值给模态框中的文本框、将类型ID赋值给模态框中的 隐藏域
...
$("#addBtn").click(function(){
	...
	// 2、清空文本框和 隐藏域 的值 --- 添加与修改共用一个模态框，所以需要先清空模态框文本域中的值
	$("#typeId").val("");
	$("#typeName").val("");
	// 清空提示信息
	$("#msg").html("");
