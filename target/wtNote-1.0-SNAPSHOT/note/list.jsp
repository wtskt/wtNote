            ...
            <nav style="text-align: center">
                <ul class="pagination   center">
                    <c:if test="${page.pageNum > 1}">
                        <li>
                            <a href="index?actionName=${action}&pageNum=${page.prePage}&title=${title}&date=${date}&typeId=${typeId}">
                                <span>«</span>
                            </a>
                        </li>
                    </c:if>
                    ...
                    <c:forEach begin="${page.startNavPage}" end="${page.endNavPage}" var="p">
                        ...
                        <li <c:if test="${page.pageNum == p}"> class="active" </c:if> >
                            <a href="index?actionName=${action}&pageNum=${p}&title=${title}&date=${date}&typeId=${typeId}">${p}</a>
                        </li>
                    </c:forEach>

                    <c:if test="${page.pageNum < page.totalPages}">
                        <li>
                            <a href="index?actionName=${action}&pageNum=${page.nextPage}&title=${title}&date=${date}&typeId=${typeId}">
                                <span>»</span>
                            </a>
                        </li>
                    </c:if>
